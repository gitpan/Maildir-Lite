# This Makefile is for the Maildir::Lite extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Maildir/Lite.pm]
#     AUTHOR => q[Deian Stefan <stefan@cooper.edu>]
#     NAME => q[Maildir::Lite]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::More=>q[0.01], File::Sync=>q[0.09], Sys::Hostname=>q[1.11] }
#     VERSION_FROM => q[lib/Maildir/Lite.pm]
#     clean => { FILES=>q[Maildir-Lite-*] }
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Maildir/Lite.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Maildir
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Maildir
...
TO_INST_PM = lib/Maildir/Lite.pm

PM_TO_BLIB = lib/Maildir/Lite.pm \
...
DISTNAME = Maildir-Lite
DISTVNAME = Maildir-Lite-0.01
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Maildir/Lite.pm $(INST_MAN3DIR)/Maildir::Lite.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:         Maildir-Lite' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:      0.01' >> META_new.yml
	$(NOECHO) $(ECHO) 'version_from: lib/Maildir/Lite.pm' >> META_new.yml
	$(NOECHO) $(ECHO) 'installdirs:  site' >> META_new.yml
	$(NOECHO) $(ECHO) 'requires:' >> META_new.yml
	$(NOECHO) $(ECHO) '    File::Sync:                    0.09' >> META_new.yml
	$(NOECHO) $(ECHO) '    Sys::Hostname:                 1.11' >> META_new.yml
...
ppd:
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>A very simple implementation of Maildir</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Deian Stefan &lt;stefan@cooper.edu&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="File-Sync" VERSION="0,09,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Sys-Hostname" VERSION="1,11,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Test-More" VERSION="0,01,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' \
	  lib/Maildir/Lite.pm blib/lib/Maildir/Lite.pm 
